...
class AnalyzerJet(Analyzer):
    ...
    def __init__(self, datap, case, typean, period):
        ...
        self.compare_with_inclusive = datap["analysis"][self.typean]["compare_with_inclusive"]
        if self.compare_with_inclusive:
            self.d_resultsinclusive = datap["analysis"][typean]["mc"]["resultsinclusive"]
            self.n_fileeff_inc = os.path.join(self.d_resultsinclusive, "efficiencies.root")
    ...
    def makeratio(self, origin_histo, option, lc_histoname):
        ...
        leg_ratio.Draw("same")
        c_ratio.SaveAs("%s/Lc+D0_combined_plot_%s.png" % (self.d_resultsallpdata , lc_histoname))
        ...
        lc_histo_sc.SetYTitle("#Lambda_{c} / D_{0}  ratio")
        lc_histo_sc.Draw()
        c_ratio.SaveAs("%s/Lc_D0_ratio_%s.png" % (self.d_resultsallpdata, lc_histoname))
        del lc_histo_orig
        del d0_histo
        del lc_histo
        del lc_histo_sc
    ...
    def fit(self): # pylint: disable=too-many-branches, too-many-locals
        ...
        for ipt in range(self.p_nptfinbins):
            ...
            for ibin2 in range(self.p_nbin2_reco):
                ...
                c_mc_fit.SaveAs("%s/fit_mc_%s.png" % (self.d_resultsallpdata, suffix_plot))
                ...
                if (np.isnan(bkg) == False and np.isnan(sig) == False and np.isnan(s_to_b) == False):
                    ...
                    latex2 = TLatex(0.71, 0.67, "%g #leq #it{p}_{T, %s} < %g GeV/#it{c}" % \
                        (self.lpt_finbinmin[ipt], self.p_latexnhadron, min(self.lpt_finbinmax[ipt], self.lvar2_binmax_reco[ibin2])))
                    draw_latex(latex2)
                c_fitted_result.SaveAs("%s/fit_%s.png" % (self.d_resultsallpdata, suffix_plot))
        myfilemc.Close()
        myfile.Close()
        fileout.Close()
    ...
    def efficiency(self):
        ...
        if self.compare_with_inclusive:
            lfileeff_inc = TFile.Open(self.n_fileeff_inc)
            if not lfileeff_inc:
                self.logger.fatal(make_message_notfound(self.n_fileeff_inc))
        ...
        legeff.Draw()
        cEff.SaveAs("%s/efficiency_pr.png" % self.d_resultsallpdata)
        ...
        legeffFD.Draw()
        cEffFD.SaveAs("%s/efficiency_fd.png" % self.d_resultsallpdata)

        if self.compare_with_inclusive:
            effhisto_inc_pr = lfileeff_inc.Get("eff_mult0")
            effhisto_inc_fd = lfileeff_inc.Get("eff_fd_mult0")
            nbins = effhist_part_pr.GetNbinsX()
            xlow = effhist_part_pr.GetXaxis().GetBinLowEdge(1)
            xhigh = effhist_part_pr.GetXaxis().GetBinUpEdge(nbins)
            inc_eff_pr = TH1F("Inclusive efficiency prompt", "", nbins, xlow, xhigh)
            inc_eff_fd = TH1F("Inclusive efficiency nonprompt", "", nbins, xlow, xhigh)
            for ipt in range(self.p_nptfinbins):
                print(self.lpt_finbinmin[ipt], effhisto_inc_pr.FindBin(self.lpt_finbinmin[ipt]))
                print(self.lpt_finbinmin[ipt], effhist_part_pr.FindBin(self.lpt_finbinmin[ipt]))
                eff_pr_inc = effhisto_inc_pr.GetBinContent(effhisto_inc_pr.FindBin(self.lpt_finbinmin[ipt]))
                eff_prompt = effhist_part_pr.GetBinContent(effhist_part_pr.FindBin(self.lpt_finbinmin[ipt]))
                eff_pr = eff_pr_inc/eff_prompt
                print(eff_pr_inc, eff_prompt, eff_pr)
                inc_eff_pr.SetBinContent(ipt+1, eff_pr)
            ...
            setup_legend(legeff_ratio)
            y_min_h, y_max_h = get_y_window_his(inc_eff_fd, inc_eff_pr)
            y_margin_up = 0.005
            y_margin_down = 0.005
            y_min, y_max = get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
            setup_histogram(inc_eff_pr, get_colour(1), 1)
            inc_eff_pr.Draw("")
            legeff_ratio.AddEntry(inc_eff_pr, "prompt_eff/prompt_eff_inclusive" , "LE")
            inc_eff_pr.SetTitle("")
            inc_eff_pr.SetXTitle("#it{p}_{T}^{%s} (GeV/#it{c})" % self.p_latexnhadron)
            inc_eff_pr.SetYTitle("efficiency ratio")
            inc_eff_pr.GetYaxis().SetRangeUser(y_min, y_max)
            inc_eff_pr.SetTitleOffset(1.2, "Y")
            inc_eff_pr.SetTitleOffset(1.1, "X")
            setup_histogram(inc_eff_fd, get_colour(2), 2)
            inc_eff_fd.Draw("same")
            legeff_ratio.AddEntry(inc_eff_fd, "fd_eff/fd_eff_inclusive" , "LE")
            legeff_ratio.Draw("same")
            cEff_ratio.SaveAs("%s/efficiency_ratio.png" % self.d_resultsallpdata)

        #cEff_pr = TCanvas("cEff_pr", "The Fit Canvas")
        #setup_canvas(cEff_pr)
        #legeff_pr = TLegend(.13, .65, .5, .88)
        #setup_legend(legeff_pr)
        #y_min_h, y_max_h = get_y_window_his(effhisto_inc_pr, effhist_part_fd)
        #y_min_h = 0
        #y_margin_up = 0.35
        #y_margin_down = 0
        #y_min, y_max = get_plot_range(y_min_h, y_max_h, y_margin_down, y_margin_up)
        #setup_histogram(effhist_part_pr, get_colour(1), 1)
        #effhist_part_pr.Draw("")
        #legeff_pr.AddEntry(effhist_part_pr, "prompt_eff" , "LE")
        #effhist_part_pr.SetTitle("")
        #effhist_part_pr.SetXTitle("#it{p}_{T}^{%s} (GeV/#it{c})" % self.p_latexnhadron)
        #effhist_part_pr.SetYTitle("efficiency")
        #effhist_part_pr.GetYaxis().SetRangeUser(y_min, y_max)
        #effhist_part_pr.SetTitleOffset(1.2, "Y")
        #effhist_part_pr.SetTitleOffset(1.1, "X")
        #setup_histogram(effhist_part_fd, get_colour(2), 2)
        #effhisto_inc_pr.Draw("same")
        #legeff_pr.AddEntry(effhisto_inc_pr, "prompt_eff_inclusive" , "LE")
        #legeff_pr.Draw("same")
        #cEff_pr.SaveAs("%s/efficiency_pr+inc.png" % self.d_resultsallpdata)
    ...
    def sideband_sub(self):
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            for ipt in range(self.p_nptfinbins):
                ...
                if int_n > 0 and int_d > 0:
                    ...
                    hzbkgratio.Draw()
                    line.Draw("same")
                csblr.SaveAs("%s/sideband_left_right_%s.png" % (self.d_resultsallpdata, suffix_plot))
                ...
                latex2 = TLatex(0.5, 0.83,
                                ...
                                % (self.lpt_finbinmin[ipt], self.p_latexnhadron, min(self.lpt_finbinmax[ipt], self.lvar2_binmax_reco[ibin2])))
                draw_latex(latex2)
                csubz.SaveAs("%s/sideband_sub_effcorr_%s.png" % (self.d_resultsallpdata, suffix_plot))
                ...
                if logscale:
                    csigbkgsubz.SetLogy()
                csigbkgsubz.SaveAs("%s/sideband_sub_%s.png" % \
            ...
            latex = TLatex(0.2, 0.83, "%g #leq %s < %g GeV/#it{c}" % \
                           (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            cz.SaveAs("%s/sideband_sub_effcorr_ptint_%s.png" % \
            ...
            crelsig.SaveAs("%s/sideband_relsig_%s.png" % (self.d_resultsallpdata, suffix))
            ...
            draw_latex(latex, textsize=0.04)
            crelunc.SaveAs("%s/sideband_relunc_%s.png" % (self.d_resultsallpdata, suffix))
            ...
            draw_latex(latex, textsize=0.04)
            cworth.SaveAs("%s/sideband_worth_%s.png" % (self.d_resultsallpdata, suffix))
            ...
            draw_latex(latex, textsize=0.04)
            cimpincl.SaveAs("%s/sideband_impincl_%s.png" % (self.d_resultsallpdata, suffix))
            ...
            draw_latex(latex, textsize=0.04)
            cimpexcl.SaveAs("%s/sideband_impexcl_%s.png" % (self.d_resultsallpdata, suffix))
            ...
            draw_latex(latex, textsize=0.04)
            cimptest.SaveAs("%s/sideband_imptest_%s.png" % (self.d_resultsallpdata, suffix))
        ...
        hzvsjetpt.Draw("text")
        czvsjetpt.SaveAs("%s/sideband_output.png" % self.d_resultsallpdata)
        fileouts.Close()
    ...
    def feeddown(self):
        ...
        cgen_eff.SaveAs("%s/kineff_fd_gen.png" % self.d_resultsallpdata)
        ...
        creco_eff.SaveAs("%s/kineff_fd_rec.png" % self.d_resultsallpdata)
        ...
        cresponse_fd.SaveAs("%s/response_fd_matrix.png" % self.d_resultsallpdata)
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            cz_genvsreco.SaveAs("%s/response_fd_%s_%s.png" % \
        ...
        for ibinshape in range(self.p_nbinshape_reco):
            ...
            cjetpt_genvsreco.SaveAs("%s/response_fd_%s_%s.png" % \
        ...
        cz_genvsreco.SaveAs("%s/response_fd_%s_full.png" % \
        ...
        cjetpt_genvsreco.SaveAs("%s/response_fd_%s_full.png" % \
        ...
        cz_genvsreco_real.SaveAs("%s/response_fd_%s_full_real.png" % \
                            (self.d_resultsallpdata, self.v_varshape_binning))
        ...
        cjetpt_genvsreco_real.SaveAs("%s/response_fd_%s_full_real.png" % \
            (self.d_resultsallpdata, self.v_var2_binning))
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            latex2 = TLatex(0.15, 0.78, "PYTHIA 8, pp, #sqrt{#it{s}} = 13 TeV")
            draw_latex(latex2)
            ...
            draw_latex(latex6)
            ceff.SaveAs("%s/efficiency_pr_fd_%s.png" % (self.d_resultsallpdata, suffix_plot))
        ...
        cjetpt_fracdiff.SaveAs("%s/response_fd_reldiff_%s.png" % (self.d_resultsallpdata, self.v_var2_binning))
        ...
        cz_fracdiff.SaveAs("%s/response_fd_reldiff_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning))
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            latex = TLatex(0.6, 0.8, "%g #leq %s < %g GeV/#it{c}" % \
                    (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            c_fd_fold.SaveAs("%s/feeddown_folded_%s.png" % (self.d_resultsallpdata, suffix_plot))
        ...
        folded.Draw("text")
        cfeeddown_2d.SaveAs("%s/feeddown_folded.png" % self.d_resultsallpdata)
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            latex = TLatex(0.6, 0.8, "%g #leq %s < %g GeV/#it{c}" % \
                (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            cfeeddown.SaveAs("%s/feeddown_subtraction_%s.png" % \
                             (self.d_resultsallpdata, suffix_plot))
            ...
            latex = TLatex(0.6, 0.8, "%g #leq %s < %g GeV/#it{c}" % \
               (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            cfeeddown_fraction.SaveAs("%s/feeddown_fraction_%s.png" % \
                                      (self.d_resultsallpdata, suffix_plot))
        ...
        cfeeddown_output.SaveAs("%s/feeddown_output.png" % self.d_resultsallpdata)
    ...
    def unfolding(self):
        ...
        input_mc_det = unfolding_input_file.Get("sample_closure")
        if not input_mc_det:
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            draw_latex(latex)
            c_mc_reco_gen_matched_z_ratio.SaveAs("%s/reco_gen_matched_%s_ratio_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            latex = TLatex(0.2, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            c_mc_reco_gen_matched_z.SaveAs("%s/reco_gen_matched_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            hz_genvsreco_list[ibin2].SetYTitle("%s^{rec}" % self.v_varshape_latex)
            hz_genvsreco_list[ibin2].Draw("colz")
            cz_genvsreco.SaveAs("%s/response_pr_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
        ...
        for ibinshape in range(self.p_nbinshape_reco):
            ...
            hjetpt_genvsreco_list[ibinshape].SetYTitle("#it{p}_{T, jet}^{rec} (GeV/#it{c})")
            hjetpt_genvsreco_list[ibinshape].Draw("colz")
            cjetpt_genvsreco.SaveAs("%s/response_pr_%s_%s.png" % (self.d_resultsallpdata, self.v_var2_binning, suffix_plot))
        ...
        cresponse_pr.SaveAs("%s/response_pr_matrix.png" % self.d_resultsallpdata)
        ...
        hjetpt_genvsreco_full.SetYTitle("#it{p}_{T, jet}^{rec} (GeV/#it{c})")
        hjetpt_genvsreco_full.Draw("colz")
        cjetpt_genvsreco_full.SaveAs("%s/response_pr_%s_full.png" % (self.d_resultsallpdata, self.v_var2_binning))
        ...
        hjetpt_genvsreco_full_real.SetYTitle("#it{p}_{T, jet}^{rec} (GeV/#it{c})")
        hjetpt_genvsreco_full_real.Draw("colz")
        cjetpt_genvsreco_full_real.SaveAs("%s/response_pr_%s_full_real.png" % (self.d_resultsallpdata, self.v_var2_binning))
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            latex = TLatex(0.2, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            ckinematic_eff.SaveAs("%s/kineff_pr_gen_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
        ...
        draw_latex(latex)
        ckinematic_eff_jetpt.SaveAs("%s/kineff_pr_gen_%s.png" % (self.d_resultsallpdata, self.v_var2_binning))
        ...
        hzvsjetpt_gen_eff.SetYTitle("#it{p}_{T, jet}^{gen} (GeV/#it{c})")
        hzvsjetpt_gen_eff.Draw("text")
        cgen_eff.SaveAs("%s/kineff_pr_gen.png" % self.d_resultsallpdata)
        ...
        hzvsjetpt_reco_eff.SetYTitle("#it{p}_{T, jet}^{rec} (GeV/#it{c})")
        hzvsjetpt_reco_eff.Draw("text")
        creco_eff.SaveAs("%s/kineff_pr_rec.png" % self.d_resultsallpdata)
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            hjetpt_fracdiff_list[ibin2].Draw("same")
        leg_jetpt_fracdiff.Draw("same")
        cjetpt_fracdiff.SaveAs("%s/response_pr_reldiff_%s.png" % (self.d_resultsallpdata, self.v_var2_binning))
        ...
        for ibinshape in range(self.p_nbinshape_gen):
            ...
            hz_fracdiff_list[ibinshape].Draw("same")
        leg_z_fracdiff.Draw("same")
        cz_fracdiff.SaveAs("%s/response_pr_reldiff_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning))
        ...
        for i in range(self.niter_unfolding):
            ...
            for ibin2 in range(self.p_nbin2_gen):
                ...
                draw_latex(latex2)
                cunfolded_z.SaveAs("%s/unfolded_%s_%d_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, i + 1, suffix_plot))
                ...
                if i == i_iter_choice:
                    ...
                    cunfolded_z.SaveAs("%s/unfolded_%s_sel_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            draw_latex(latex2)
            cunfolded_jetpt.SaveAs("%s/unfolded_%s_%d.png" % (self.d_resultsallpdata, self.v_var2_binning, i + 1))
            ...
            for ibin2 in range(self.p_nbin2_reco):
                ...
                latex2 = TLatex(0.5, 0.82, "iteration %d" % (i + 1))
                draw_latex(latex2)
                cfolded_z.SaveAs("%s/refolding_%s_%d_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, i + 1, suffix_plot))
            ...
            latex2 = TLatex(0.5, 0.82, "iteration %d" % (i + 1))
            draw_latex(latex2)
            cfolded_jetpt.SaveAs("%s/refolding_%s_%d.png" % (self.d_resultsallpdata, self.v_var2_binning, i + 1))
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            latex = TLatex(0.15, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_gen[ibin2], self.p_latexbin2var, self.lvar2_binmax_gen[ibin2]))
            draw_latex(latex)
            cconvergence_z.SaveAs("%s/convergence_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            draw_latex(latex)
            cinput_mc_gen_z.SaveAs("%s/unfolded_vs_mc_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            cinput_mc_gen_z_xsection.SaveAs("%s/unfolded_vs_mc_%s_xsection_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            draw_latex(latex)
            cconvergence_refolding_z.SaveAs("%s/convergence_refolding_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            draw_latex(latex)
            cunfolded_not_z.SaveAs("%s/unfolded_not_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix_plot))
            ...
            latex = TLatex(0.2, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_reco[ibin2], self.p_latexbin2var, self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex)
            cunfolded_not_stat_error.SaveAs("%s/unfolded_not_stat_error_%s.png" % (self.d_resultsallpdata, suffix_plot))
        ...
        draw_latex(latex)
        cconvergence_jetpt.SaveAs("%s/convergence_%s.png" % (self.d_resultsallpdata, self.v_var2_binning))
    ...
    def unfolding_closure(self):
        ...
        for i in range(self.niter_unfolding):
            ...
            for ibin2 in range(self.p_nbin2_gen):
                ...
                latex2 = TLatex(0.5, 0.82, "iteration %d" % (i + 1))
                draw_latex(latex2)
                cclosure_z.SaveAs("%s/closure_%s_%d_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, i + 1, suffix_plot))
            ...
            latex2 = TLatex(0.5, 0.82, "iteration %d" % (i + 1))
            draw_latex(latex2)
            cclosure_jetpt.SaveAs("%s/closure_%s_%d.png" % (self.d_resultsallpdata, self.v_var2_binning, i + 1))
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            latex = TLatex(0.15, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_gen[ibin2], self.p_latexbin2var, self.lvar2_binmax_gen[ibin2]))
            draw_latex(latex)
            cconvergence_closure_z.SaveAs("%s/convergence_closure_%s_%s.png" % (self.d_resultsallpdata, self.v_varshape_binning, suffix))
        ...
        latex = TLatex(0.15, 0.82, "%g #leq %s < %g" % (round(self.lvarshape_binmin_gen[0], 2), self.v_varshape_latex, round(self.lvarshape_binmax_gen[-1], 2)))
        draw_latex(latex)
        cconvergence_closure_jetpt.SaveAs("%s/convergence_closure_%s.png" % (self.d_resultsallpdata, self.v_var2_binning))
    ...
    def jetsystematics(self):
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            if self.lc_d0_ratio:
                ...
                print(input_powheg_lc)
                print(input_powheg_z[ibin2])
                ...
                print(shapebins_centres_array, shapebins_contents_array, shapebins_widths, shapebins_error)
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            csysvar.SaveAs("%s/sys_var_all_%s.png" % (self.d_resultsallpdata, suffix))
            ...
            for sys_cat in range(self.n_sys_cat):
                ...
                csysvar_each.SaveAs("%s/sys_var_%s_%s.png" % (self.d_resultsallpdata, suffix2, suffix))
                ...
                y_min, y_max = get_y_window_his(histo_ratio)
                y_margin_up = 0.15
                y_margin_down = 0.05
                ...
                y_min, y_max = get_y_window_his(input_histograms_sys_eff[ibin2][sys_cat] + [eff_default[ibin2]])
                y_margin_up = 0.15
                y_margin_down = 0.05
                nsys = 0
                for sys_var in range(self.systematic_variations[sys_cat]):
                    ...
                    if input_histograms_sys_eff[ibin2][sys_cat][sys_var].GetBinContent(1) > 0:
                       ...
                       input_histograms_sys_eff[ibin2][sys_cat][sys_var].Reset()
                       setup_histogram(input_histograms_sys_eff[ibin2][sys_cat][sys_var], 0, get_marker(nsys + 1))
                    nsys = nsys + 1
                latex = TLatex(0.15, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_gen[ibin2], self.p_latexbin2var, self.lvar2_binmax_gen[ibin2]))
                draw_latex(latex)
                ...
                for r_val in range(len(self.lvarshape_binmin_gen)):
                    setup_histogram(var_histos[r_val], get_colour(nsys + 1), get_marker(nsys + 1))
                    if r_val == 0:
                        var_histos[r_val].GetYaxis().SetRangeUser(*get_plot_range(y_min, y_max, y_margin_down, y_margin_up))
                        var_histos[r_val].SetTitle("")
                        var_histos[r_val].SetXTitle("number of %s variation" %suffix2)
                        var_histos[r_val].SetYTitle("")
                        var_histos[r_val].Draw("hist")
                    leg_varhisto.AddEntry(var_histos[r_val], labels[r_val], "P")
                    var_histos[r_val].Draw("same hist")
                    #line = TLine()
                    #line.SetLineColor(get_colour(nsys+1))
                    #line.DrawLine(0, def_array[r_val],
                    #        self.systematic_variations[sys_cat],
                    #        def_array[r_val])
                    #var_histos[r_val]._line = line
                    nsys = nsys+1
                latex = TLatex(0.15, 0.82, "%g #leq %s < %g GeV/#it{c}" % (self.lvar2_binmin_gen[ibin2], self.p_latexbin2var, self.lvar2_binmax_gen[ibin2]))
                draw_latex(latex)
            ...
            for ibinshape in range(self.p_nbinshape_gen):
                ...
                for sys_cat in range(self.n_sys_cat):
                    ...
                    for sys_var in range(self.systematic_variations[sys_cat]):
                        ...
                        #    if self.systematic_catnames[sys_cat] == "cutvar":
                        #        print("AAAA!WHY I'M HERE????")
...
#                leg_finalwsys_wmodels = TLegend(.55, .52, .65, .72)
            ...
            crelativesys.SaveAs("%s/sys_unc_%s.png" % (self.d_resultsallpdata, suffix))
            if ibin2 == 1:
        ...
        for ibin2 in range(self.p_nbin2_reco):
            ...
            #draw_latex(latex7)
            cfeeddown_fraction.SaveAs("%s/feeddown_fraction_var_%s.png" % (self.d_resultsallpdata, suffix_plot))
    ...
    def get_soft_mode_0_weighed(self, file_path: str, file_path_1: str, file_path_2: str, dim: int, prompt: bool):
        ...
        print("Entries after filtering:", len(df_sim))
        print("Entries after filtering:", df_sim.shape)
        # Create, fill and scale the histogram.
        print("Filling a %dD histogram" % dim)
        if dim == 2:
            # Binning: x - shape, y - jet pt
